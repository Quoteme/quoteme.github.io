...
The hardware for this experiment was my trusty old ThinkPad X230T: A machine 
I'd assume is more likely to survive a nuclear apocalypse than a cockroach.
And more importantly, it was capable of taking the beating I gave it constantly:
...
I worked on this repository almost every other day for four years. The `modules/hardware/laptops/` folder gives us useful insights for hardware usage (changes there almost exclusively reflect hardware-level adjustments). Hover over any laptop entry in the file tree to see its commit timeline highlighted.

For a more precise picture of any given period, select a file node and zoom into the commit graph. Individual commit messages are highlighted then to indicate when this file was changed.

Hovering over `./xmonad` for example reveals a continuous stretch of development, ending abruptly on February 4th, 2023, when I [removed Xmonad from my config](https://github.com/Quoteme/nixos/commit/ea730ec). A quiet end to a chapter.
...
Publishing my system configuration openly is does come with dangers.
My public NixOS config exposes my hardware inventory, software stack, and commit cadence. 
Why do I not care about that?
I share mine because I make sure to share nothing sensitive: 
secrets are not managed by NixOS and never enter the repository, credentials are never hardcoded, and all hardware-identifying information is either innocuous or intentionally public.
...
The following checks cover the three most common accidental disclosure vectors in repositories:
...
The results above show, concretely, what an attacker could find out from the public commit history.
The diff scan is most thorough: we search
through code, even if it was subsequently deleted.
