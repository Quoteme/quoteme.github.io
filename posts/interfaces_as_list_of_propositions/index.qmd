...
subtitle: |
    Or: Applying the Howard-Curry correspondence to OOP interfaces using the calculus of constructions
author: "Luca Leon Happel"
date: "2025-05-31 15:20"
categories: ["mathematics", "category theory", "oop", "English"]
format:
    html:
        code-fold: true
...
Looking closely at the definition of an interface, we can see that it is
basically a list of methods/attributes which must be explicitly implemented.
...
without any arguments). We may embed Java's (or any other) type system into
...
Let's get back to the interface example above. Here we don't provide any function
...
_of name $n_f$_ therefore corresponds to the proposition: $n_f(X) = T$.
...
corresponds to a proof. We implicitly used this above already, by stating
...
Now, finding a proof for the validity of $n_f(X)$ is therefore the same as
constructing a term $t$ of type $n_f(X)$. This $t$ would therefore
...
(after some sufficient embedding of Java's type system). The same holds true for
`(){return 42;}`, which is of type `IO Int`.

## Further outlook

Due to Java's (and most other OOP languages') type systems' limitations, or the
limits of computation, the following interfaces sadly are not entirely
realizable (albeit one could find some nifty ways around the respective
restrictions of the type system). These interfaces are nonetheless of utmost
importance:
...
interface Group<T> {
    ...
    // Therefore we reduce this step to pseudo code:
...
our program to the outer layers, like the database or the user interface.
