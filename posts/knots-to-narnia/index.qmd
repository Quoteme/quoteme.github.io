...
def build_rope_segments(progress, n_pts=100):
    ...
    A margin keeps the rope geometry away from the portal disk so no
    spheres leak through to the other side."""
    cx_L, cx_R = -2.0, 2.0
    margin = 0.22          # keep well clear of portal disk
    x_start, x_end = -5.0, 5.0
    amp = 0.12

    x_stop_L  = cx_L - margin          # rope stops before left portal
    x_start_R = cx_R + margin          # rope starts after right portal
...
def sdf_rope_seg(p, seg, n_vis, radius):
    """SDF for a rope segment rendered as union of spheres."""
    if n_vis <= 0:
        return 1e10
    d_min = 1e10
        ...
        dp = min(sdf_torus_rotated(pp, cx_L, torus_R, torus_r),
                 min(sdf_torus_rotated(pp, cx_R, torus_R, torus_r),
                     min(sdf_rope_seg(pp, seg1, n1, rope_r),
                         min(sdf_rope_seg(pp, seg2, n2, rope_r),
        ...
        dm = min(sdf_torus_rotated(pm, cx_L, torus_R, torus_r),
                 min(sdf_torus_rotated(pm, cx_R, torus_R, torus_r),
                     min(sdf_rope_seg(pm, seg1, n1, rope_r),
                         min(sdf_rope_seg(pm, seg2, n2, rope_r),
